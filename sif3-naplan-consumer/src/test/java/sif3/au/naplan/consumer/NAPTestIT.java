...
public class NAPTestIT {
    NAPTestConsumer consumer;

    @Before
    public void setup() {
        ConsumerLoader.initialise("NaplanConsumer");
        consumer = new NAPTestConsumer();
    }
    ...
    public void testConsumer() {
        try {
            ...
            Assert.assertTrue("Response body correct type", NAPTestCollectionType.class.isAssignableFrom(responses.get(0).getDataObject().getClass()));
            Assert.assertTrue("Response body contains NAPTests", NAPTestCollectionType.class.cast(responses.get(0).getDataObject()).getNAPTest().size() > 0);
        } catch (Exception ex) {
