...
public class NAPEventStudentLinkIT {
    NAPEventStudentLinkConsumer consumer;

    @Before
    public void setup() {
        ConsumerLoader.initialise("NaplanConsumer");
        consumer = new NAPEventStudentLinkConsumer();
    }
    ...
    public void testConsumer() {
        try {
            ...
            Assert.assertTrue("Response body correct type", NAPEventStudentLinkCollectionType.class.isAssignableFrom(responses.get(0).getDataObject().getClass()));
            Assert.assertTrue("Response body contains NAPEventStudentLinks", NAPEventStudentLinkCollectionType.class.cast(responses.get(0).getDataObject()).getNAPEventStudentLink().size() > 0);
        } catch (Exception ex) {
