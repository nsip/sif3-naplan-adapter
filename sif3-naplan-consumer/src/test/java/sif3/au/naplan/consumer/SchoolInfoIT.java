...
public class SchoolInfoIT {
    SchoolInfoConsumer consumer;

    @Before
    public void setup() {
        ConsumerLoader.initialise("NaplanConsumer");
        consumer = new SchoolInfoConsumer();
    }
    ...
    public void testConsumer() {
        try {
            ...
            Assert.assertTrue("Response body correct type", SchoolInfoCollectionType.class.isAssignableFrom(responses.get(0).getDataObject().getClass()));
            Assert.assertTrue("Response body contains SchoolInfos", SchoolInfoCollectionType.class.cast(responses.get(0).getDataObject()).getSchoolInfo().size() > 0);
        } catch (Exception ex) {
