...
public class SchoolDataIT {
    SchoolDataConsumer consumer;

    @Before
    public void setup() {
        ConsumerLoader.initialise("NaplanConsumer");
        consumer = new SchoolDataConsumer();
    }
    ...
    public void testConsumer() {
        try {
            ...
            Assert.assertTrue("Response body contains Students", NaplanResponse.class.cast(responses.get(0).getDataObject()).getBody().contains("<StudentPersonal "));
            Assert.assertTrue("Response body contains NAPEventStudentLink", NaplanResponse.class.cast(responses.get(0).getDataObject()).getBody().contains("<NAPEventStudentLink "));
            Assert.assertTrue("Response body contains NAPStudentResponseSet", NaplanResponse.class.cast(responses.get(0).getDataObject()).getBody().contains("<NAPStudentResponseSet "));
            Assert.assertTrue("Response body contains NAPTestScoreSummary", NaplanResponse.class.cast(responses.get(0).getDataObject()).getBody().contains("<NAPTestScoreSummary "));
