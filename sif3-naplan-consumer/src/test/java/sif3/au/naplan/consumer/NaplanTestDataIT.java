...
public class NaplanTestDataIT {
    TestDataConsumer consumer;
    ...
    public void testConsumer() {
        try {
            ...
            Assert.assertTrue("Response body contains NAPCodeFrame", NaplanResponse.class.cast(responses.get(0).getDataObject()).getBody().contains("<NAPCodeFrame "));
            Assert.assertTrue("Response body contains NAPTest", NaplanResponse.class.cast(responses.get(0).getDataObject()).getBody().contains("<NAPTest "));
            Assert.assertTrue("Response body contains NAPTestlet", NaplanResponse.class.cast(responses.get(0).getDataObject()).getBody().contains("<NAPTestlet "));
            Assert.assertTrue("Response body contains NAPTestItem", NaplanResponse.class.cast(responses.get(0).getDataObject()).getBody().contains("<NAPTestItem "));
        } catch (Exception ex) {
